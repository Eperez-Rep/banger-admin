    ...
    fetch(CSV_URL)
      ...
      .then(function(text) {
        ...
        var datos = rows.map(function(r, idx) {
          var num = parseFloat(String(r.precio || "").replace(/[^0-9.]/g, "")) || 0;
          return {
            id:        idx + 1,
            nombre:    r.nombre || "",
...
/* =====================
   ADMIN PANEL
   ===================== */
var SHEETDB_URL   = "https://sheetdb.io/api/v1/jo10ckzatecba";
...
function adminAgregarProducto() {
  ...
  var imagen   = document.getElementById("adm-imagen").value.trim();
  var tipo     = document.getElementById("adm-tipo").value;
  var genetica = document.getElementById("adm-genetica").value.trim();
  ...
  if (!marcaValida) {
    adminMostrarError(msg, "✕ Marca no permitida. Usá una de las marcas autorizadas.");
    return;
  }
  ...
  if (!precio || isNaN(precioNum) || precioNum <= 0) {
    adminMostrarError(msg, "⚠ Precio inválido. Ingresá un número mayor a 0.");
    return;
  }

  /* Validar imagen */
  if (imagen && imagen.indexOf(IMG_BASE) !== 0) {
    adminMostrarError(msg, "✕ URL de imagen inválida. Debe comenzar con: " + IMG_BASE);
    return;
  }
  ...
  fetch(SHEETDB_URL, {
    ...
    body: JSON.stringify({ data: [{ nombre: nombre, precio: precioNum, imagen: imagen, tipo: tipo, genetica: genetica }] })
  })
  .then(function(r) { return r.json(); })
  .then(function() {
    adminMostrarOk(msg, "✓ Producto agregado correctamente.");
    ["adm-nombre","adm-precio","adm-imagen","adm-genetica"].forEach(function(id) {
  ...
  .catch(function() {
    adminMostrarError(msg, "✕ Error al conectar con SheetDB.");
    btn.textContent = "Agregar producto";
    btn.disabled = false;
  });
}
